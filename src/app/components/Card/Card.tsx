...
const Card: React.FC<SingleCardTemplate> = ({
  disabled,
  card,
  onClick,
}) => {
  const clickHandler = () => {
    if (!disabled || card.cardState === "matched") {
      onClick();
    }
  };
  
  let flipped = false
  if (card.cardState === "matched" || card.cardState === "flipped") {
    flipped = true
  }
  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-0 h-full w-full [transform:rotateY(180deg)] [backface-visibility:hidden]">
        <Image className="rounded-lg" data-cy="front" src={card.url} alt={card.alt||'front'} fill sizes="90vw" />
      </div>
    </div>
  );
};
