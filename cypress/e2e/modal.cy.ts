describe("modal managment", () => {
  ...
  it("should change the difficulty level correctly", () => {
    cy.visit('http://localhost:3000/');
    cy.get('[data-cy="diff-level"] [data-cy="button"]').click();
    cy.get('[data-cy="select-level"]').select('8');
    cy.get('[data-cy="difficulty"] [data-cy="submit"]').click();
    cy.get('[data-cy="back-drop"]').should('not.exist')
    cy.get('[data-cy="modal"]').should('not.exist')
    cy.get('[data-cy="level"]').contains('8')
    cy.get('[data-cy="moves"]').contains('0')
    cy.get('[data-cy="cards-container"]').should('have.class', 'grid-cols-8');
    cy.get('[data-cy="cards-container"]').find('[data-cy="back"]').should('have.length', 64)
    cy.get('[data-cy="cards-container"] div.absolute [data-cy="front"]').should('have.length', 64)
